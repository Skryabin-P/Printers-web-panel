...
def view_full_report(response):
    ...
    q = filter.qs.values("printer").annotate(ip = F('printer__ip'),
                                                        ...
                                                        cartridges = Sum('changed'),mean = Coalesce(Case(When(cartridges__gt=1,changed=1, then=((Max('page_utility') - Min('page_utility')) / (F('cartridges')-1))),
        output_field=CharField()),Value('1')), device_name = F('device_name'),
